...
function renderTrendingByCategory() {
  ...
  const groups = Object.entries(TRENDING_CAT_META).map(([key, meta]) => {
    const repos = CURATED_TRENDING.filter(r => r.cat === key);
    if (!repos.length) {
      return `<div class="trending-category-group">
        <div class="trending-category-heading"><span>${meta.icon}</span>${escHtml(meta.label)}<span class="trending-cat-count">0</span></div>
        <p style="font-size:13px;color:var(--text-muted);padding:8px 0;">Coming soon — resources being added.</p>
      </div>`;
    }
    return `<div class="trending-category-group">
      <div class="trending-category-heading"><span>${meta.icon}</span>${escHtml(meta.label)}<span class="trending-cat-count">${repos.length}</span></div>
